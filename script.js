...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.openChat = name => {
    ...
    if (area) area.innerHTML=`<div style="text-align:center;opacity:.5;font-size:12px;padding:16px">Loading...</div>`;
  ...
  function attachListeners() {
    ...
    // ── Chat messages (FIX-G: room + conversation filtered) ───────────────
    onSnapshot(
      ...
      snap => {
        ...
        snap.forEach(d => {
          const msg = d.data();
          const inConvo = (msg.from===currentUser&&msg.to===chattingWith)||(msg.from===chattingWith&&msg.to===currentUser);
          if (!inConvo) return;
          ...
          if (msg.to===currentUser && msg.status==="sent")    updateDoc(doc(db,"messages",d.id),{status:"delivered"}).catch(()=>{});
          if (msg.to===currentUser && msg.from===chattingWith) updateDoc(doc(db,"messages",d.id),{status:"seen"}).catch(()=>{});
        });
