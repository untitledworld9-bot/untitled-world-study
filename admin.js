...
const COLL = {
  ...
  ANALYTICS:     "analytics",
  MUSIC:         "musicTracks"   // FEAT-3: admin-managed music tracks
};
...
function renderTimerLeaderboard(container) {
  ...
  if (!users.length) {
    container.innerHTML = `<div class="empty-state">
      ...
      <div class="empty-state-text">No focus time recorded yet</div>
    </div>`;
    return;
  }
      ...
      <tbody>
        ${users.map((u, i) => {
          ...
          <tr style="border-bottom:1px solid var(--border);">
            ...
            <td style="padding:12px;font-family:var(--font-mono);color:var(--accent-cyan);">${formatFocusTime(u.focusTime || 0)}</td>
            <td style="padding:12px;font-family:var(--font-mono);color:var(--accent-amber);">⚡ ${xp}</td>
            <td style="padding:12px;font-size:12px;">${badge}</td>
            <td style="padding:12px;">${statusBadge(u.status)}</td>
          </tr>`;
