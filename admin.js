...
function renderOnlineUsersModal() {
  ...
  listEl.innerHTML = list.map(u => {
    const pageName = u.currentPage || u.page || u.activePage || "Unknown Page";
    const pageLabel = (() => {
      if (pageName.includes("timer"))    return "⏱️ Focus Timer";
      if (pageName.includes("todo"))     return "✅ To-Do";
      if (pageName.includes("playlist")) return "🎵 Playlist";
      if (pageName.includes("home") || pageName === "index" || pageName === "home")
                                         return "🏠 Home";
      return `📄 ${pageName}`;
    })();
    ...
    ">
      ...
      <div style="display:flex;gap:6px;align-items:center;flex-shrink:0;">
        ...
        " onmouseover="this.style.background='rgba(0,224,255,.25)'" onmouseout="this.style.background='rgba(0,224,255,.1)'">🔔</button>
        <button title="Send Promotion" onclick="document.getElementById('onlineUsersModal').classList.remove('open');onlineUserQuickAction('promotion','${uid}','${uname}')" style="
          background:rgba(124,92,252,.1);border:1px solid rgba(124,92,252,.2);
...
function renderFullLeaderboard(container) {
  // Include ALL users — even those with 0 XP (they still have a level and badge)
  const users = [...STATE.allUsers]
    .sort((a, b) => {
