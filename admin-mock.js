...
window.createTest = async () => {
  ...
  try {
    ...
    if (exam === 'jee') {
      testData.subjects = {
        physics:   parseInt($('ctPhyQ').value) || 30,
        chemistry: parseInt($('ctChemQ').value) || 30,
        maths:     parseInt($('ctMathQ').value) || 30
      };
    }
...
window.publishTest = async () => {
  ...
  try {
    ...
    // Update test status
    const testRef = doc(db, COLL.TESTS, testId);
...
window.deleteTest = async id => {
  ...
  try {
    ...
    batch.delete(doc(db, COLL.TESTS, id));
    await batch.commit();
...
async function loadMaintenance() {
  try {
    const snap = await getDoc(doc(db, COLL.MAINTENANCE, 'config'));
...
window.saveMaintenance = async () => {
  ...
  try {
    await setDoc(doc(db, COLL.MAINTENANCE, 'config'), {
